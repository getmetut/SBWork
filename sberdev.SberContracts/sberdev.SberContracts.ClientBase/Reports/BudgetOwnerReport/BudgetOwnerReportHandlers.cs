...
{
  partial class BudgetOwnerReportClientHandlers
  {

    public override void BeforeExecute(Sungero.Reporting.Client.BeforeExecuteEventArgs e)
    {
      if (!BudgetOwnerReport.StartDate.HasValue && !BudgetOwnerReport.EndDate.HasValue)
      {
        ...
        if (dialog.Show() == DialogButtons.Ok)
        {
          ...
          BudgetOwnerReport.StartDate = startDate.Value.Value;
          BudgetOwnerReport.EndDate = endDate.Value.Value;
          if (type.Value == SberContracts.Reports.Resources.PaidInvoiceReport.PaidInvoiceReportDialog_Expendable)
            BudgetOwnerReport.Type = "Expendable";
          else
            BudgetOwnerReport.Type = "Profitable";
        }
        else
          e.Cancel = true;
      }
