...
namespace sberdev.SberContracts.Server
{
  public partial class ModuleInitializer
  {
    ...
    {
      Sungero.Docflow.PublicInitializationFunctions.Module.CreateDocumentType("Заявка на RnD закупку", AppRnDPurchase.ClassTypeGuid, Sungero.Docflow.DocumentType.DocumentFlow.Inner, true);
    ...
    public void CreateDocumentKinds()
    {
      ...
      Sungero.Docflow.PublicInitializationFunctions.Module.CreateDocumentKind("Заявка на RnD закупку", "Заявка на RnD закупку",
                                                                              Sungero.Docflow.DocumentKind.NumberingType.Numerable,
                                                                              Sungero.Docflow.DocumentType.DocumentFlow.Inner, true, true,
                                                                              AppRnDPurchase.ClassTypeGuid, new Sungero.Domain.Shared.IActionInfo[]
                                                                              {Sungero.Docflow.OfficialDocuments.Info.Actions.SendForApproval},
                                                                              Constants.Module.AppRnDPurchaseGuid);
      // Создание вида документа «Заявка на производ. закупку».
    ...
    public static void GrantCreateRightsOnInfoDocument()
    {
      ...
      SberContracts.AppRnDPurchases.AccessRights.Grant(allUsers, DefaultAccessRightsTypes.Create);
      ...
      SberContracts.AppRnDPurchases.AccessRights.Save();
    ...
    public void CreateDevSettings()
    {
      ...
      settingsNames.Add("Настройка рассылки подзадач по контролю возврата", "Процесс просматривает задания на контроль возврата. Если заданию больше {Текстовый параметр 1} дней," +
                        " он отсылает подзадачу инициатору задачи. Каждые {Текстовый параметр 3} по созданной подзадаче будут отсылаться напоминания ее исполнителю." +
                        " Если еще {Текстовый параметр 2} дней нет ответа от инициатора, подзадача ликвидируется. Если через {Текстовый параметр 1}" +
                        " дней задания на контроль возврата все еще в процессе, подзадача создастся сново. В свойстве Контрагенты нужно указать для каких ка процесс будет делать исключения.");
      settingsNames.Add("Путь к папке с логами", "В текстовом параметре нужно указать путь к папке с логами. В ");
      ...
      settingsNames.Add("ИД сущностей для договора Xiongxin", "В текстовом параметре нужно указать через запятую: " +
      ...
      settingsNames.Add("Виды документов с обязательным полем \"Номер 1С\"",
      ...
      foreach(var settingName in settingsNames)
      {
        ...
        if (devSet == null)
        {
          ...
          if (Equals(settingName.Key, "Подразделения с обязательным полем \"Номер 1С\""))
            devSet.Text = "51,437";
          if (Equals(settingName.Key, "Виды документов с обязательным полем \"Номер 1С\""))
            devSet.Text = "56,36,27";
          devSet.Save();
        }
      }
    }
